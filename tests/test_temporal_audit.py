...
def test_temporal_coverage_respects_each_horizon_and_asset_boundaries():
    rows = [
        {"source_id": "A", "captured_at": "2026-01-01T00:00:00Z"},
        {"source_id": "A", "captured_at": "2026-01-01T00:30:00Z"},
        {"source_id": "A", "captured_at": "2026-01-01T02:00:00Z"},
        {"source_id": "A", "captured_at": "2026-01-01T10:00:00Z"},
        {"source_id": "B", "captured_at": "2026-01-01T00:05:00Z"},
    ]
    result = audit_temporal_coverage(rows, horizons_hours=(1, 6, 24))
    assert result["1h"]["eligible"] == 2
    assert result["6h"]["eligible"] == 3
    assert result["24h"]["eligible"] == 4
